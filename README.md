...
**Для ПРОДАКШЕНА:**
```bash
./super-reliable-deploy.sh  # Развернуть
./check-deployment.sh       # Проверить
```

**Для РАЗРАБОТКИ:**
```bash
./quick-deploy.sh      # Развернуть
./check-deployment.sh  # Проверить
```

### 📖 ПОДРОБНЫЙ СПРАВОЧНИК
**Все скрипты и инструкции:** [`SCRIPTS.md`](SCRIPTS.md)

### 🎯 РЕКОМЕНДАЦИИ
- **Всегда используйте `./super-reliable-deploy.sh` для продакшена**
- **Проверяйте результат через `./check-deployment.sh`**
- **При проблемах: `./health-check.sh` → `./rollback.sh`**
