...
describe('BuyAtClose4Simulator with Real GOOGL Data', () => {
  ...
  beforeEach(() => {
    ...
    // Convert to OHLCData format - dates are already YYYY-MM-DD strings
    googlData = parsedData.data.map((item: any) => ({
      date: item.date, // TradingDate string
      open: item.open,
      high: item.high,
      low: item.low,
      close: item.close,
  ...
  describe('data validation', () => {
    ...
    it('should have valid OHLC structure', () => {
      ...
      expect(typeof firstBar.date).toBe('string'); // TradingDate
    ...
    it('should have chronological order', () => {
      for (let i = 1; i < googlData.length; i++) {
        expect(compareTradingDates(googlData[i].date, googlData[i - 1].date)).toBe(1);
      }
    });
