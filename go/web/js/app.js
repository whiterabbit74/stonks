(() => {
  ...
  const PATHS = {
    ...
    webull: '<path d="M2.4 6.3A10.2 8.6 0 0 0 21.6 6.3" fill="none" stroke="currentColor" stroke-width="4.8" stroke-linecap="round"/>',
  ...
  function statusBadgeHTML() {
    ...
    if (st === 'offline') {
      return `<span id="footer-status" title="Офлайн" class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-red-100 text-red-800 dark:bg-red-950/30 dark:text-red-200"><span class="w-1.5 h-1.5 bg-red-500 rounded-full"></span>Офлайн</span>`;
    }
    if (st === 'checking') {
      return `<span id="footer-status" title="Проверка" class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"><span class="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>Проверка</span>`;
    }
      return `<span id="footer-status" title="Онлайн" class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-green-100 text-green-800 dark:bg-green-950/30 dark:text-green-200"><span class="w-1.5 h-1.5 bg-green-500 rounded-full"></span>Онлайн</span>`;
  }
    ...
    if (!state.datasets.length) {
      ...
      cards = `<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-2">${filtered.map((d) => {
        ...
        return `<div class="relative h-full">
          ...
          <button data-menu="${esc(d.ticker)}" class="icon-btn absolute top-2 right-2 h-8 w-8 text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700" title="Открыть меню действий" aria-label="Открыть меню действий">${icon('more', 'w-4 h-4')}</button>
          ${state.menuTicker === d.ticker ? `<div class="absolute right-2 top-14 z-10 w-40 rounded-lg border bg-white shadow dark:bg-gray-800 dark:border-gray-700 text-sm">
    ...
    if ((tab === 'overview' || tab === 'positions' || tab === 'orders' || tab === 'fills') && !dash) {
      ...
      const pendingRows = tracked.map((o) => {
        ...
        const actions = stuck && o.clientOrderId
          ...
          : '';
        return `<tr class="${stuck ? 'bg-amber-50 dark:bg-amber-950/30' : ''}"><td>${esc(o.symbol || '')}</td><td>${esc(brokerLabel(o.broker))}</td><td>${esc(orderSideText(o.action))}</td><td class="${stuck ? 'font-semibold text-amber-700 dark:text-amber-400' : ''}">${esc(trackerStatusText(o.status))}</td><td>${esc(o.quantity ?? '')}</td><td>${esc(formatDateTimeET(o.startedAt || o.started_at || ''))}</td><td>${actions}</td></tr>`;
      }).join('');
  ...
  async function paintSplitsForTickers(el, tickers) {
    ...
    try {
      ...
      el.innerHTML = `<div class="text-sm text-gray-500 mb-2">Всего сплитов: ${total}</div><div class="grid md:grid-cols-2 lg:grid-cols-3 gap-3">${cards.join('')}</div>`;
