...
export function Results() {
  ...
  type ChartTab = 'price' | 'equity' | 'buyhold' | 'drawdown' | 'trades' | 'profit' | 'duration' | 'openDayDrawdown' | 'margin' | 'noStopLoss' | 'buyAtClose' | 'splits';
  ...
  // Load trading calendar once (public endpoint; no auth required)
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        const res = await fetch('/api/trading-calendar', { cache: 'no-store' });
        if (!res.ok) throw new Error('calendar_load_failed');
        const json = await res.json();
        if (alive) setTradingCalendar(json);
      } catch {
        if (alive) setTradingCalendar(null);
      }
    })();
    return () => { alive = false; };
  }, []);
