...
Открыть Agentic Account и сделать **первый** логин агента Robinhood разрешает **только с
десктопного браузера** (`robinhood-capabilities.md` §10). Это ручной шаг оператора, один раз.
...
* Глубина: по постановке задачи ~5 лет. **Это надо подтвердить эмпирически на этапе 2**
...
### 1.5 PDT (обязательно проговорить с пользователем)

Стратегия может входить и выходить в один день. На маржинальном счёте < $25 000 действует
правило pattern day trader: не более 3 day trades за 5 рабочих дней. `review_equity_order`
возвращает pre-trade alerts, включая PDT. **Не игнорировать их** — см. этап 6.
...
> ⚠️ **Ловушка развёртывания.** Redirect идёт на loopback **браузера пользователя**, а не сервера.
> Если сервер крутится на удалённой машине/в докере, `127.0.0.1:53682` в браузере оператора никуда
> не попадёт. Два выхода, выбрать один и записать в README:
> **(A)** делать OAuth локально (на ноутбуке оператора поднять сервер, получить refresh_token,
> перенести его на прод через `PUT /api/autotrade/robinhood/token` — по аналогии с
> `PUT /autotrade/webull/token`);
> **(B)** зарегистрировать в DCR публичный HTTPS-redirect на домен приложения
> (`https://<домен>/api/autotrade/robinhood/oauth/callback`) и ловить код прямо там.
> **(B) чище и рекомендуется**, если у приложения уже есть HTTPS-домен (Caddy в docker-compose есть).
> Если Robinhood откажет на HTTPS-redirect в DCR — падаем на (A).
> Это первое, что надо проверить на этапе 2.
...
2. **Где живёт OAuth-redirect** — HTTPS-домен приложения или локальный ноутбук (§2.2, вариант A/B)?
...
4. **PDT:** счёт Robinhood Agentic — cash или margin, и меньше ли $25k? От этого зависит,
   надо ли ограничивать число входов в неделю.
