...
func TestLoginFailsWhenSessionSetIsBlocked(t *testing.T) {
	...
	if out["error"] != "Не удалось создать сессию" {
...
func TestLoginUnknownUserDoesPasswordWork(t *testing.T) {
	...
	if !bytes.Contains(rec.Body.Bytes(), []byte(`"error":"Неверный логин или пароль"`)) {
...
func TestTestProviderUnknownAndNoKeyLeak(t *testing.T) {
	...
	for _, p := range []string{"alpha_vantage", "finnhub", "twelve_data", "polygon", "webull"} {
		...
		rec = httptest.NewRecorder()
		s.Handler().ServeHTTP(rec, req)
		if rec.Code == 400 && strings.Contains(rec.Body.String(), "Неизвестный провайдер") {
