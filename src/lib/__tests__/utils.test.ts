...
describe('Utils', () => {
  ...
  describe('dedupeDailyOHLC', () => {
    it('should remove duplicate dates', () => {
      const data = [
        { date: '2023-12-25', open: 100, high: 110, low: 90, close: 105, volume: 1000 },
        { date: '2023-12-25', open: 105, high: 115, low: 95, close: 110, volume: 2000 },
        { date: '2023-12-26', open: 110, high: 120, low: 100, close: 115, volume: 1500 }
      ];

      const result = dedupeDailyOHLC(data);
      expect(result).toHaveLength(2);
      // TradingDate is YYYY-MM-DD string
      expect(result[0].date).toBe('2023-12-25');
      expect(result[1].date).toBe('2023-12-26');
    });
  ...
  describe('adjustOHLCForSplits', () => {
    it('should adjust prices for stock splits', () => {
      const data = [
        { date: '2023-12-20', open: 100, high: 110, low: 90, close: 105, volume: 1000 },
        { date: '2023-12-25', open: 105, high: 115, low: 95, close: 110, volume: 2000 },
        { date: '2023-12-30', open: 110, high: 120, low: 100, close: 115, volume: 1500 }
      ];
    ...
    it('should handle multiple splits', () => {
      ...
      // First entry: 100 / 2 / 1.5 = 33.33 (both splits apply because bar is before both)
      expect(result[0].open).toBeCloseTo(33.33, 2);

      // Second entry: 105 / 1.5 = 70 (only second split applies because bar is after first split)
      expect(result[1].open).toBeCloseTo(70, 2);

      // Third entry: 110 (no split applies because bar is after both splits)
