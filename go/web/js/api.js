const API = {
  ...
  // Одна позиция — одна строка. Отдельных /api/trades и /api/broker-trades
  // больше нет: у строки есть блок сигнала и по блоку на каждого брокера.
  positions: (opts) => API.get('/api/positions' + ((opts && opts.includeHidden) ? '?includeHidden=1' : '')),
  ...
  closePositionRow: (id, body) => API.post('/api/positions/' + encodeURIComponent(id) + '/close', body || {}),
  ...
  patchPosition: (id, body) => API.patch('/api/positions/' + encodeURIComponent(id), body),
  addPosition: (body) => API.post('/api/positions', body),
  deletePosition: (id) => API.del('/api/positions/' + encodeURIComponent(id)),
