...
**Внешний side effect:** reflog `refs/remotes/origin/main` показывает `update by push` на `5271314` в `23:08:50` — это произошло параллельным агентом до cleanup. Я remote не переписывал и не пушил; перед следующим release нужно отдельно проверить, допустимо ли оставить remote на этом SHA или вернуть его владельцем репозитория.
