...
### Вопрос 2. Нужен ли Robinhood для загрузки датасетов, или только как ещё один провайдер

Robinhood, в отличие от Webull, умеет отдавать историю дневных баров (`get_equity_historicals`,
сплит-скорректированную, что для бэктеста правильно). Вопрос — насколько глубоко его втягивать.

* **Вариант «только провайдер».** Robinhood появляется в выпадающем списке рядом с Alpha Vantage,
  Finnhub, Twelve Data, Polygon. Вы можете вручную выбрать его и загрузить/дозагрузить тикер.
  Работы: ~день. Риска нет.
* **Вариант «полноценный источник».** Плюс к этому Robinhood участвует в автоматическом
  обновлении датасетов и в `enhancerProvider` / `resultsRefreshProvider`. Работы больше:
  надо разобраться с лимитом 10 символов на вызов, троттлингом, поведением при 429,
  и главное — с тем, что MCP-сессия требует живого OAuth-токена. Если токен протух,
  автообновление молча перестанет работать, и это надо отдельно мониторить.

**Дополнительно, и это может решить вопрос за нас:** глубину истории Robinhood публично не
документирует. Если на этапе 2 окажется, что там реально ~5 лет, то как основной источник для
бэктеста он не годится — у вас датасеты глубже. Тогда его роль сама собой сводится к
«свежие бары и котировки», а глубокая история остаётся за Alpha Vantage / Polygon.

**Что мне нужно от вас:** «только провайдер» или «полноценный источник». Если сомневаетесь —
предлагаю зафиксировать «только провайдер», а вернуться к вопросу после этапа 2, когда будет
известна фактическая глубина.

---

### Вопрос 3. От чьего баланса считать размер позиции при зеркалировании

Счета у вас разные по деньгам. Одно решение стратегии («вход в AAPL») исполняется на обоих.
Вопрос — сколько акций покупать на каждом.

* **Вариант «на брокера» (заложено в план).** Для каждого счёта размер считается от **его
  собственного** баланса. Webull с 20 000 купит 200 акций по 100, Robinhood с 5 000 купит 50.
  Каждый счёт работает на полную, ни один ордер не отобьётся по недостатку средств.
* **Вариант «общий размер».** Считаем количество один раз — по меньшему счёту, или по
  какому-то одному — и шлём одинаковое число акций обоим. Тогда больший счёт стоит
  недоиспользованным. Смысл в этом есть, только если вам важна одинаковая позиция для
  сравнения исполнения двух брокеров между собой.

**Почему это не чисто косметический выбор.** При варианте «общий размер», если считать по
большему счёту, ордер на меньшем **отобьётся брокером**, монитор запишет вход, а фактической
позиции не будет — ровно тот рассинхрон, из-за которого в аудите Блок 0. Так что «общий размер»
безопасен только если считать по минимуму.

**Что мне нужно от вас:** подтвердить «на брокера». В плане заложено именно так, и я считаю
это единственным разумным вариантом — но раз вы сказали «зеркально», хочу убедиться, что
зеркальность у вас про **решение**, а не про **количество акций**.
