...
Активный Webull token становится `INVALID` только после 15 последовательных дней без аутентифицированных API-запросов. Поэтому сервер каждый календарный день делает keep-alive: сначала сверяет статус через `token/check`, затем при `NORMAL` выполняет аутентифицированный запрос account balance (либо `account/list`, если account ID не задан). Проверка запускается на каждом цикле scheduler, но сама дедуплицирует успешный день и ограничивает повтор после сбоя. Локальная дата `expires_at` — только метаданные от Webull: она никогда не блокирует использование токена со статусом `NORMAL`.
...
Сервер сохраняет токен в SQLite и присылает напоминание в Telegram за 3 дня до срока, сообщённого Webull, а также при статусе проверки, отличном от `NORMAL`. Неуспешный keep-alive не отмечается завершённым и повторяется с ограничением частоты в 15 минут.
